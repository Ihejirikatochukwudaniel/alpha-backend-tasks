...
async def get_briefing_endpoint(briefing_id: UUID, session: AsyncSession = Depends(get_session)):
...
async def generate_report_endpoint(briefing_id: UUID, session: AsyncSession = Depends(get_session)):
...
async def get_html_endpoint(briefing_id: UUID, session: AsyncSession = Depends(get_session)):
    briefing = await get_briefing(session, briefing_id)
    if not briefing or not briefing.is_generated:
        raise HTTPException(status_code=404, detail="Report not generated")
    return briefing.html_content

async def get_full_briefing_response(session: AsyncSession, briefing_id: UUID) -> BriefingResponse | None:
    from sqlalchemy import select
    from app.models.briefing import BriefingPoint, BriefingMetric
    stmt = select(Briefing).where(Briefing.id == briefing_id)
    result = await session.execute(stmt)
    briefing = result.scalar_one_or_none()
    if not briefing:
        return None
    kp_stmt = select(BriefingPoint.content).where(BriefingPoint.briefing_id == briefing_id, BriefingPoint.type == 'key_point').order_by(BriefingPoint.display_order)
    r_stmt = select(BriefingPoint.content).where(BriefingPoint.briefing_id == briefing_id, BriefingPoint.type == 'risk').order_by(BriefingPoint.display_order)
